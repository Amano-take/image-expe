...
class Batch():
    ...
    def study(self):
        ...
        W_filter = np.random.normal(loc=0, scale=0.01, size=(
            Batch.filterK, Batch.filterR * Batch.filterR))
        b_filter = np.repeat(np.random.normal(loc=0, scale=0.01, size=(
            Batch.filterK, 1)), imglength*imgwidth*Batch.B, axis=1)
        ...
        for i in range(num):
            ...
            for j in range(epoch):
                ...
                # 畳み込み
                arX, img = self.conv2D_prime(
                    img, W_filter, Batch.filterR, b_filter)
                # print(img.shape) -> filterK * (imsize * B)
                # print(arX.shape) -> filterR^2 * (imgsize * B)
                ...
                mganma = ganma_para[1]
                vganma = ganma_para[2]
                W_filter_para = self.Adam(
                    W_filter, delta_filter_W, t, mfilW, vfilW)
                W_filter = W_filter_para[0]
                mfilW = W_filter_para[1]
                vfilW = W_filter_para[2]
                b_filter_para = self.Adam(
                    b_filter, delta_filter_b, t, mfilb, vfilb)
            ...
            if (crossE < previouscrossE):
                np.savez("./Parameters/conv_Adam", W1,
                         W2, b1, b2, self.beta, self.ganma, W_filter, b_filter)
    ...
    def test(self):
        ...
        batch_random = np.random.randint(0, 10000, Batch.B)
        before_conv = np.array(Xtest[batch_random])
        answer = np.array(Ytest[batch_random])
        M = Batch.M
        C = Batch.C
        ...
        img = before_conv.reshape((B, img_size, 1))
        img = self.normalize_one(img)
        _, img = self.conv2D_prime(img, W_filter, Batch.filterR, b_filter)
        img = img.reshape(B, img_size, 1)
